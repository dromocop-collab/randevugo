...
export default function PricingPage() {
  ...
  return <BusinessPage className="pricing-v2"><main>
    ...
    <section className="pricing-stage">
      <article className="pricing-card"><div className="pricing-card-top"><div><span>SENİNRANDEVUN</span><h2>Her şey dahil</h2></div><b>LANSMAN KAMPANYASI</b></div><div className="price-line"><strong>0</strong><div><span>₺</span><small>/ ilk 3 ay</small></div></div><p>İlk 3 ay ücretsiz · sonrasında yılda <b>{PLAN_PRICE.yearly.toLocaleString("tr-TR")} ₺</b></p><div className="trial-line"><i /> Tüm özellikler 90 gün açık · kredi kartı gerekmez</div><Link href={signedIn ? "/dashboard" : "/isletmeler/kayit"}>{signedIn ? "Panelime devam et" : "Ücretsiz hesabını aç"}<span>↗</span></Link><div className="pricing-trust"><span>Kredi kartı yok</span><span>Kurulum ücreti yok</span><span>Kurulum desteği</span></div></article>
    ...
    <section className="pricing-faq"><h2>Sık sorulanlar</h2><div>{[["İlk 3 ay ücretsiz kampanyası nasıl çalışır?","Yeni işletme kayıtları tüm özellikleri kayıt tarihinden itibaren 3 ay ücretsiz kullanır. Kart bilgisi istenmez."],["Çalışan veya randevu limiti var mı?","250 çalışana kadar destek verilir; müşteri ve randevu sayısı sınırsızdır."],["Ücretsiz dönem bitince ne olur?",`3 ay sonunda kullanım, o tarihteki güncel abonelik koşullarına göre devam eder. Mevcut yıllık fiyat ${PLAN_PRICE.yearly.toLocaleString("tr-TR")} ₺'dir.`],["Mevcut verilerimi taşıyabilir miyim?","Müşteri listenizi aktarabilir, kurulum desteğimizden yararlanabilirsiniz."]].map(([q,a])=><details key={q}><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</div></section>
  </main></BusinessPage>;
}
