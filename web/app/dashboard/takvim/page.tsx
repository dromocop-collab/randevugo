...
export default function CalendarPage() {
  ...
  return <div className="calendar-page space-y-5">
    ...
    <section className="calendar-panel">
      <header className="calendar-toolbar"><div className="calendar-navigation"><button onClick={() => move(-1)} aria-label="Önceki dönem"><ChevronLeft size={19} /></button><button className="calendar-today" onClick={() => setCursor(startOfDay(new Date()))}>Bugün</button><button onClick={() => move(1)} aria-label="Sonraki dönem"><ChevronRight size={19} /></button><div className="calendar-title"><strong>{rangeTitle(cursor, view)}</strong><span>{appointments.length} randevu</span></div></div><div className="calendar-actions"><div className="calendar-view-switch" role="tablist" aria-label="Takvim görünümü">{(Object.keys(VIEW_LABELS) as ViewMode[]).map((item) => <button key={item} role="tab" aria-selected={view === item} className={view === item ? "active" : ""} onClick={() => setView(item)}>{VIEW_LABELS[item]}</button>)}</div><button className="calendar-add"><Plus size={17} /> <span>Yeni randevu</span></button></div></header>
...
function AppointmentDrawer({ appointment, onClose }: { appointment: Appointment; onClose: () => void }) { return <div className="calendar-drawer-backdrop" onMouseDown={onClose}><aside className="calendar-drawer" onMouseDown={(event) => event.stopPropagation()}><button className="drawer-close" onClick={onClose} aria-label="Detayı kapat">×</button><div className={`drawer-status ${STATUS[appointment.status].className}`}>{STATUS[appointment.status].label}</div><h2>{appointment.customerName}</h2><p>{appointment.serviceName || "Hizmet bilgisi girilmemiş"}</p><div className="drawer-details"><div><Clock3 size={18} /><span><small>Tarih ve saat</small><strong>{format(new Date(appointment.startAt), "d MMMM yyyy, HH:mm", { locale: tr })} – {format(new Date(appointment.endAt), "HH:mm")}</strong></span></div><div><UserRound size={18} /><span><small>Çalışan</small><strong>{appointment.staffName || "Atanmamış"}</strong></span></div><div><CreditCard size={18} /><span><small>Ödeme</small><strong>{appointment.paymentStatus === "paid" ? "Ödendi" : appointment.paymentStatus === "deposit_paid" ? "Kapora ödendi" : "Ödeme bekliyor"}</strong></span></div></div>{appointment.notes && <div className="drawer-note"><small>Randevu notu</small><p>{appointment.notes}</p></div>}<button className="drawer-primary">Randevuyu yönet</button></aside></div>; }
