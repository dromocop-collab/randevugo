...
import { BadgeCheck, CircleAlert, Plus, ReceiptText, Save, Store, UsersRound, WalletCards } from "lucide-react";
...
export default function SuperAdminSubscriptionsPage() {
  ...
  const emptyPlan = (): PlatformPlan => ({ id: "RANDEVUGO", label: PLAN_LABEL, yearlyPrice: PLAN_PRICE.yearly, monthlyPrice: PLAN_PRICE.monthlyEquivalent, currency: "TRY", trialDays: PLAN_PRICE.trialDays, maxStores: 3, maxStaff: 250, isActive: true, isRecommended: true, description: "Tüm randevu operasyonunu tek merkezden yönetin.", features: [...PLAN_FEATURE_LIST] });
  const [editing, setEditing] = useState<PlatformPlan>(emptyPlan);
  const [busy, setBusy] = useState(false);

  async function loadSubscriptions() {
    const db = getDb();
    const [subscriptionSnapshot, businessSnapshot] = await Promise.all([
      getDocs(collection(db, "subscriptions")),
      getDocs(collection(db, "businesses")),
    ]);
    const businessNames = new Map(businessSnapshot.docs.map((item) => [item.id, String(item.data().name ?? "İsimsiz işletme")]));
    setSubscriptions(subscriptionSnapshot.docs.map((item) => {
      const data = item.data();
      const businessId = String(data.businessId ?? item.id);
      return {
        id: item.id,
        businessId,
        businessName: businessNames.get(businessId) ?? "İşletme kaydı bulunamadı",
        plan: String(data.plan ?? "RANDEVUGO"),
        status: String(data.status ?? "trialing") as SubscriptionStatus,
        paymentProvider: String(data.paymentProvider ?? "manual") as PaymentProviderKey,
        renewalEnabled: data.renewalEnabled === true,
        trialEndsAt: typeof data.trialEndsAt === "string" ? data.trialEndsAt : undefined,
        subscriptionEndsAt: typeof data.subscriptionEndsAt === "string" ? data.subscriptionEndsAt : undefined,
      };
    }));
  }

  async function reload() { await Promise.all([listPlatformPlans().then(setPlans), loadSubscriptions()]); }
  useEffect(() => {
    let cancelled = false;
    Promise.all([listPlatformPlans(), (async () => {
      const db = getDb();
      const [subscriptionSnapshot, businessSnapshot] = await Promise.all([getDocs(collection(db, "subscriptions")), getDocs(collection(db, "businesses"))]);
      const names = new Map(businessSnapshot.docs.map((item) => [item.id, String(item.data().name ?? "İsimsiz işletme")]));
      return subscriptionSnapshot.docs.map((item) => {
        const data = item.data();
        const businessId = String(data.businessId ?? item.id);
        return { id: item.id, businessId, businessName: names.get(businessId) ?? "İşletme kaydı bulunamadı", plan: String(data.plan ?? "RANDEVUGO"), status: String(data.status ?? "trialing") as SubscriptionStatus, paymentProvider: String(data.paymentProvider ?? "manual") as PaymentProviderKey, renewalEnabled: data.renewalEnabled === true, trialEndsAt: typeof data.trialEndsAt === "string" ? data.trialEndsAt : undefined, subscriptionEndsAt: typeof data.subscriptionEndsAt === "string" ? data.subscriptionEndsAt : undefined } satisfies SubscriptionRow;
      });
    })()])
      .then(([planRows, subscriptionRows]) => { if (!cancelled) { setPlans(planRows); setSubscriptions(subscriptionRows); } })
      .catch(() => toast.error("Paket ve abonelik verileri yüklenemedi."));
    ...
    <div className="space-y-5">
      ...
      <section className="overflow-hidden rounded-[26px] border border-cyan-950/10 bg-white/85 shadow-sm">
        ...
        {subscriptions.length === 0 ? <p className="p-8 text-center text-sm text-slate-500">Henüz abonelik kaydı oluşmadı.</p> : <div className="divide-y divide-cyan-950/5">{subscriptions.slice(0, 12).map((item) => {
          const statusLabel: Record<SubscriptionStatus, string> = { trialing: "Denemede", active: "Aktif", past_due: "Ödeme bekliyor", cancelled: "İptal", expired: "Süresi doldu" };
