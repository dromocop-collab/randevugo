    ...
    <main className="live-discovery-main">
      ...
      <section id="canli-isletmeler" className="live-discovery-results" aria-labelledby="live-results-title">
        ...
        {enabled === null || loading ? <div className="live-state-card" role="status"><span className="live-state-card__icon"><RefreshCw size={25} className="animate-spin" /></span><h3>İşletmeler kontrol ediliyor</h3><p>Güncel canlı sıra durumunu getiriyoruz.</p></div> : !enabled ?
          ...
          <div className="live-state-card"><span className="live-state-card__icon"><Search size={27} /></span><h3>{hasFilters ? "Bu aramaya uygun işletme bulunamadı" : "Şu anda açık canlı sıra yok"}</h3><p>{hasFilters ? "Farklı bir arama veya şehir deneyebilirsin. İstersen filtreleri temizleyip tüm işletmelere bak." : "Yeni müşteri kabul eden işletmeler burada görünecek. Bu sırada normal randevu seçeneklerine göz atabilirsin."}</p><div className="live-state-card__links">{hasFilters && <button type="button" onClick={resetFilters} className="live-state-card__action">Filtreleri temizle <ArrowRight size={17} /></button>}<Link href="/kesfet" className={hasFilters ? "live-state-card__secondary" : "live-state-card__action"}>Normal randevuları keşfet <ArrowRight size={17} /></Link></div></div> : <>
