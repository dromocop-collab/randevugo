...
export function BusinessFooter() {
  return <footer className="business-footer"><div><Link href="/isletmeler" className="business-brand"><Image src="/logo.png" alt="" width={36} height={36} /><span>Senin<b>Randevun</b><small>İŞLETME</small></span></Link><p>Randevudan müşteri sadakatine, işletmenizin büyüme merkezi.</p></div><div><strong>ÜRÜN</strong><Link href="/ozellikler">Özellikler</Link><Link href="/fiyatlar">Fiyatlar</Link><Link href="/isletmeler/kayit">İlk yıl ücretsiz</Link></div><div><strong>İŞLETMENİZ</strong><Link href="/isletmeler/giris">İşletme girişi</Link><Link href="/dashboard">Yönetim paneli</Link><Link href="/isletmeler/yardim">İşletme yardımı</Link></div><div><strong>MÜŞTERİLER</strong><Link href="/">Mağaza ara</Link><Link href="/kesfet">Keşfet</Link><Link href="/yardim-merkezi">Müşteri yardımı</Link></div><div className="business-footer-bottom"><span>© {new Date().getFullYear()} SeninRandevun</span><span>KVKK odaklı · Güvenli bulut altyapısı</span></div></footer>;
}

export function BusinessPage({ children, className = "" }: { children: ReactNode; className?: string }) { return <div className={`business-marketing ${className}`}><LaunchCampaign showTicker /><BusinessHeader />{children}<BusinessFooter /></div>; }
